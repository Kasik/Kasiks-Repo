...
def QueueItem(addon_id, title, url, is_resolved=False,img='', fanart='', infolabels=''):
    ...
    pbe_script = 'XBMC.RunScript(%s, %s, %s, "%s")' % (script_path, sys.argv[1], '?' + common.dict_to_paramstr(pbe_params), 'plugin.video.megabox.playbackengine')
...
if sys.argv and len(sys.argv) >= 4 and sys.argv[3] == 'plugin.video.megabox.playbackengine':        
    
    sys.argv[0] = 'plugin.video.megabox'
    addon_pbe= Addon('plugin.video.megabox', sys.argv)
