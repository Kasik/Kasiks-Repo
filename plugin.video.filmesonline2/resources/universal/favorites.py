...
class Favorites:
    ...
    def add_item(self, title, url, fmtd_title='', section_title='Misc.', section_addon_title='', sub_section_title='', sub_section_addon_title='', infolabels='', img='', fanart='', is_playable=False, is_folder=False):
        ...
        uni_fav_add_script = 'XBMC.RunScript(%s, %s, %s, "%s")' % (self._get_script_path(), self.sys_argv[1], self._build_params(uni_fav, infolabels), 'plugin.video.filmesonline2.favorites')
    ...
    def delete_item(self, title, fmtd_title='', item_mode='main', section_title='Misc.', section_addon_title='', sub_section_title='', sub_section_addon_title=''):
        ...
        uni_fav_add_script = 'XBMC.RunScript(%s, %s, %s, "%s")' % (self._get_script_path(), self.sys_argv[1], self._build_params(uni_fav), 'plugin.video.filmesonline2.favorites')
        
        return uni_fav_add_script
    
    def _get_script_path(self):
        return os.path.join('special://home/addons/plugin.video.filmesonline2','resources','universal','favorites.py')
...
if sys.argv and len(sys.argv) >= 4 and sys.argv[3] == 'plugin.video.filmesonline2.favorites':    
    
    sys.argv[0] = 'plugin.video.filmesonline2'
    addon_fav = Addon('plugin.video.filmesonline2', sys.argv)
