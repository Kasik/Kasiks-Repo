...
if selfAddon.getSetting("artwork") == "false":
    art = xbmc.translatePath('special://home/addons/plugin.video.filmesonlinegratis/resources/art/')
elogo = xbmc.translatePath('special://home/addons/plugin.video.filmesonlinegratis/resources/art/bigx.png')
slogo = xbmc.translatePath('special://home/addons/plugin.video.filmesonlinegratis/resources/art/smallicon.png')
fanartimage=Dir+'.jpg'
...
def ErrorReport(e):
    elogo = xbmc.translatePath('special://home/addons/plugin.video.filmesonlinegratis/resources/art/bigx.png')
    xbmc.executebuiltin("XBMC.Notification([COLOR=FF67cc33]Filmesonlinegratis Error[/COLOR],"+str(e)+",10000,"+elogo+")")
    xbmc.log('***********Filmesonlinegratis Error: '+str(e)+'**************', xbmc.LOGERROR)
...
#             print status,
        ...
        if not silent:
            ...
            dialog.ok("Filmesonlinegratis", "Report the error below at " + supportsite, str(e), "We will try our best to help you")
        return False
...
def resolveDownloadLinks(url):
    if re.search('watchseries.lt',url):
        ...
        url=GetUrliW(url)
    elif re.search('filmesonlinegratis',url):
        from resources.libs import filmesonlinegratis
        url = filmesonlinegratis.resolveM25URL(url)
...
#     mi = [('Search filmesonlinegratis','XBMC.Container.Update(%s?mode=4&url=%s)'% (sys.argv[0],'###'))]
#     return addDirX(name,url,mode,iconimage,'','','',genre,year,searchMeta=1,fav_t='Movies',fav_addon_t='filmesonlinegratis',menuItemPos=0,menuItems=mi)
    return addDirX(name,url,mode,iconimage,'','','',genre,year,searchMeta=1,fav_t='Movies',fav_addon_t='filmesonlinegratis')
