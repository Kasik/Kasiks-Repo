...
def delFAVS(url,title,mode,thumb,plot,type):
    ...
    if not str(Favs).find(title):
        xbmc.executebuiltin("XBMC.Notification([B][COLOR green]VideoBull[/COLOR][/B],[B][COLOR orange]"+title+"[/COLOR]not in Favourites.[/B],1000,"")")
    ...
    else: xbmc.executebuiltin("XBMC.Notification([B][COLOR green]VideoBull[/COLOR][/B],[B]You Have No Favourites to delete[/B],1000,"")")
