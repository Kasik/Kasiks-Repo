...
def ErrorReport(e):
        ...
        xbmc.log('***********FilmesOnline2 Error: '+str(e)+'**************')
...
def addInfo(name,url,mode,iconimage,gen,year):
        ...
        if selfAddon.getSetting("meta-view") == "true":
                ...
                xbmcplugin.addSortMethod( handle=int( sys.argv[ 1 ] ), sortMethod=xbmcplugin.SORT_METHOD_LABEL )
        Commands=[
                ("[B][COLOR blue]Add[/COLOR][/B] to My Fav's",fav.add_directory(name, u, section_title='Movies', section_addon_title="filmesonline2 Fav's", img=iconimage, infolabels={'item_mode':mode, 'item_url':url, 'genre':gen,'year':year})),
                ("[B][COLOR red]Remove[/COLOR][/B] from My Fav's",fav.delete_item(name, section_title='Movies', section_addon_title="filmesonline2 Fav's"))]
        if selfAddon.getSetting("meta-view") == "true":
                video_type='movie'
                imdb=infoLabels['imdb_id']
                cname=urllib.quote_plus(infoLabels['metaName'])
