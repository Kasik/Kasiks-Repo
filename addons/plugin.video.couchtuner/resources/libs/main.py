...
couchtunerpath = selfAddon.getAddonInfo('path')
grab = None
fav = False
...
#             print status,
        ...
        if not silent:
            ...
            dialog.ok("couchtuner", "Report the error below", str(e), "We will try our best to help you")
        return False
...
def QuietDownload(url, dest,originalName, videoname):
    ...
    script = os.path.join( couchtunerpath, 'resources', 'libs', "DownloadInBackground.py" )
...
def addDirX(name,url,mode,iconimage,plot='',fanart='',dur=0,genre='',year='',isFolder=True,searchMeta=False,addToFavs=True,
    ...
    Commands.append(('[B][COLOR=FF67cc33]couchtuner[/COLOR] Settings[/B]','XBMC.RunScript('+xbmc.translatePath(couchtunerpath + '/resources/libs/settings.py')+')'))
    if menuItemPos != None:
...
def addDown4(name,url,mode,iconimage,plot,fanart,dur,genre,year):
    ...
    if re.search('(?i)\ss(\d+)e(\d+)',name) or re.search('(?i)Season(.+?)Episode',name):
        return addDirX(name,url,mode,iconimage,plot,fanart,dur,genre,year,isFolder=f,searchMeta=1,metaType='TV',
                       addToFavs=f,fav_t='TV',fav_addon_t='TV Episode',fav_sub_t='Episodes',down=1)
    else:
        return addDirX(name,url,mode,iconimage,plot,fanart,dur,genre,year,isFolder=f,searchMeta=1,addToFavs=f,
