...
darewallpath = selfAddon.getAddonInfo('path')
...
def OPEN_URL(url):
    req = urllib2.Request(url)
    req.add_header('User-Agent', 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3')
    #req.add_header('Referer', 'http://eztv.it/showlist/')

    response = urllib2.urlopen(req)
    link=response.read().replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','').replace('\\','')
    #.replace('http://www.couchtuner.eu/','http://www.zzstream.li/').replace('season-','s').replace('episode-','e')
    response.close()
    return link
...
def ErrorReport(e):
        elogo = xbmc.translatePath('special://home/addons/plugin.video.daretv/resources/art/bigx.png')
        xbmc.executebuiltin("XBMC.Notification([COLOR=FF67cc33]The DareTv Error[/COLOR],"+str(e)+",10000,"+elogo+")")
        xbmc.log('***********The DareTv Error: '+str(e)+'**************')
...
def QuietDownload(url, dest,originalName, videoname):
    ...
    script = os.path.join( darewallpath, 'resources', 'libs', "DownloadInBackground.py" )
