...
art = xbmc.translatePath('special://home/addons/plugin.video.daretv/resources/art/')
fanart=None
fanartimage=None

elogo = xbmc.translatePath('special://home/addons/plugin.video.daretv/resources/art/bigx.png')
slogo = xbmc.translatePath('special://home/addons/plugin.video.daretv/resources/art/smallicon.png')
...
def ErrorReport(e):
    elogo = xbmc.translatePath('special://home/addons/plugin.video.daretv/resources/art/bigx.png')
    xbmc.executebuiltin("XBMC.Notification([COLOR=FF67cc33]DareTv Error[/COLOR],"+str(e)+",10000,"+elogo+")")
    xbmc.log('***********DareTv Error: '+str(e)+'**************', xbmc.LOGERROR)
...
#             print status,
        ...
        if not silent:
            ...
            dialog.ok("DareTv", "Report the error below at " + supportsite, str(e), "We will try our best to help you")
        return False
...
def doRegex(url):
    ...
    import urllib2
    url=re.compile('([^<]+)<regex>',re.DOTALL).findall(url)[0]
    doRegexs = re.compile('\$doregex\[([^\]]*)\]').findall(url)
    for k in doRegexs:
        if k in url:
            regex=re.compile('<name>'+k+'</name><expres>(.+?)</expres><page>(.+?)</page><referer>(.+?)</referer></regex>',re.DOTALL).search(url)
...
def resolveDownloadLinks(url):
    if re.search('watchseries.lt',url):
        ...
        url=GetUrliW(url)
    elif re.search('DareTv',url):
        from resources.libs import DareTv
        url = DareTv.resolveM25URL(url)
    elif '</t><sec>' in url:
...
def addDirX(name,url,mode,iconimage,plot='',fanart='',dur=0,genre='',year='',imdb='',tmdb='',isFolder=True,searchMeta=False,addToFavs=True,
    ...
    Commands.append(('[B][COLOR=FF67cc33]DareTv[/COLOR] Settings[/B]','XBMC.RunScript('+xbmc.translatePath(darepath + '/resources/libs/settings.py')+')'))
    if menuItemPos != None:
...
def addInfo(name,url,mode,iconimage,genre,year):
    mi = [('Search DareTv','XBMC.Container.Update(%s?mode=4&url=%s)'% (sys.argv[0],'###'))]
